...
const newWorkout = ref<Omit<Workout, 'id' | 'userId' | 'createdAt' | 'updatedAt'>>({
  name: '',
  date: new Date(),
  type: 'strength',
  duration: 60,
...
<template>
  <div class="space-y-6 animate-slide-in">
      ...
      <form @submit.prevent="handleAddWorkout" class="space-y-4">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            ...
            <input v-model="newWorkout.name" type="text" class="input" required placeholder="Leg Day" />
          </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div>
            ...
            <input v-model.number="newWorkout.duration" type="number" min="1" class="input" />
          </div>
        </div>
            ...
            >
              ...
              <div class="grid grid-cols-1 md:grid-cols-4 gap-3">
                <div class="md:col-span-2">
                  <input v-model="exercise.name" type="text" class="input" placeholder="Exercise name" required />
                </div>
                <div>
                  <input v-model.number="exercise.sets" type="number" min="1" class="input" placeholder="Sets" required />
                </div>
                <div>
                  <input v-model.number="exercise.reps" type="number" min="1" class="input" placeholder="Reps" required />
                </div>
              </div>
              
              <div class="grid grid-cols-1 md:grid-cols-2 gap-3 mt-2">
                <div>
                  <input v-model.number="exercise.weight" type="number" min="0" class="input" placeholder="Weight (optional)" />
                </div>
                <div>
                  <input v-model.number="exercise.duration" type="number" min="0" class="input" placeholder="Duration (optional)" />
                </div>
              </div>
            </div>
          </div>
        ...
        <div>
          ...
          <textarea v-model="newWorkout.notes" class="input h-24" placeholder="How did the workout feel?"></textarea>
        </div>
    ...
    <!-- Workout History -->
    <div class="space-y-8">
      <div v-for="(workouts, month) in workoutsByMonth" :key="month" class="space-y-4">
        ...
        <div class="space-y-4">
          ...
          >
            <div class="flex justify-between items-start">
              <div>
                <h4 class="text-lg font-medium text-gray-900">{{ workout.name }}</h4>
                <p class="text-sm text-gray-500">{{ formatWorkoutDate(workout.date) }}</p>
              </div>
              <div class="flex items-center space-x-2">
                <span class="inline-block px-2 py-1 text-xs rounded-full bg-primary-100 text-primary-800">
                  {{ workout.type }}
                </span>
              ...
              <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                ...
                >
                  ...
                  <div class="text-gray-500">
                    ...
                    <span v-if="exercise.duration"> for {{ exercise.duration }}min</span>
                  </div>
                </div>
              </div>
            </div>
      ...
      <div v-if="Object.keys(workoutsByMonth).length === 0" class="text-center p-8 bg-white rounded-lg shadow">

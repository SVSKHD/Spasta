...
const emit = defineEmits<{
  (e: 'close'): void;
  (e: 'save', category: Omit<Category, 'id' | 'userId' | 'createdAt' | 'updatedAt'>): void;
}>();
      ...
      <DialogPanel class="w-full max-w-md rounded-lg bg-card p-6 shadow-xl">
        ...
        <form @submit.prevent="handleSubmit" class="space-y-4">
          <div>
            ...
            <input v-model="newCategory.name" type="text" class="input" required />
          </div>
          
          <div>
            <label class="block text-sm font-medium text-text/60">Description</label>
            <input v-model="newCategory.description" type="text" class="input" />
          </div>
          
          <div>
            <label class="block text-sm font-medium text-text/60">Color</label>
            <div class="flex items-center space-x-2">
              <input v-model="newCategory.color" type="color" class="h-8 w-8 p-0 border-0" />
              <input v-model="newCategory.color" type="text" class="input flex-grow" />
            </div>
          </div>

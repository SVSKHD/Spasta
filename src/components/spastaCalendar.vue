...
const filteredTasks = computed(() => {
  ...
  return props.tasks.filter(
    (task) => task.categoryId === selectedCategoryId.value,
  );
});
...
const selectedCategory = computed(() => {
  ...
  return (
    categoryStore.categories.find(
      (c) => c.id === selectedTask.value?.categoryId,
    ) || null
  );
});

const getRecurringTaskOccurrences = (
  task: Task,
  startDate: Date,
  endDate: Date,
) => {
...
const onDragStart = (event: DragEvent, task: Task) => {
  ...
  if (event.dataTransfer) {
    ...
    dragImage.className =
      "fixed pointer-events-none bg-card p-2 rounded shadow-lg opacity-90 transform rotate-2";
    ...
    <div class="grid grid-cols-7 gap-1">
      ...
      <div
        ...
        :class="{
          ...
          'ring-2 ring-primary-500 bg-primary-50/20 dark:bg-primary-900/20':
            activeDropDate && isSameDay(activeDropDate, day),
        }"

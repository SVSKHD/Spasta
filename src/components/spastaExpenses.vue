...
const sortedExpenses = computed(() => {
  return [...filteredExpenses.value].sort((a, b) => b.date.getTime() - a.date.getTime());
});
...
<template>
  <div class="space-y-6 animate-slide-in">
    ...
    <div v-if="showAddForm" class="bg-white rounded-lg shadow-md p-6 animate-slide-up">
      ...
      <form @submit.prevent="handleAddExpense" class="space-y-4">
        ...
        <div>
          ...
          <input v-model="newExpense.description" type="text" class="input" required />
        </div>
        ...
        <div v-if="newExpense.isRecurring">
          <label class="block text-sm font-medium text-gray-700">Recurring Period</label>
    ...
    <div class="bg-white rounded-lg shadow-md p-6">
      ...
      <div class="mb-6">
        <div class="text-3xl font-bold text-gray-900">{{ formatCurrency(totalExpenses) }}</div>
        <div class="text-sm text-gray-500">
          {{ filterCategory === 'all' ? 'Total expenses' : `Total ${filterCategory} expenses` }}
        </div>
      </div>

...
const notes = computed(() => {
  ...
  return noteStore.notesByCategory(selectedCategoryId.value).filter((note) => {
...
watch(
  [
    () => newNote.value.remarks,
    () => newNote.value.tags,
    () => newNote.value.keywords,
  ],
  triggerAutoSave,
  { deep: true },
);
    ...
    <div v-else class="page-content flex flex-col">
      ...
      <div class="flex items-center justify-between mb-6">
        <div class="flex-1 max-w-md relative">
          <Search
            class="w-4 h-4 absolute left-3 top-1/2 -translate-y-1/2 text-text/40"
          />
          <input
            v-model="searchQuery"
            type="text"
        ...
        <div class="flex items-center space-x-4">
          <div class="text-sm text-text/60">{{ notes.length }} notes</div>
      ...
      <div class="flex flex-1 gap-6 min-h-[600px]">
        ...
        <div class="w-80 flex-none overflow-hidden">
          <div class="grid grid-cols-1 gap-4">
            <div
              ...
              :class="
                note.id === selectedNoteId ? 'ring-2 ring-primary-500' : ''
              "
            ...
            >
              ...
              <div
                class="text-sm text-text/60 line-clamp-3 mb-3"
                v-html="note.content"
              ></div>
        ...
        <!-- Editor Area -->
        <div
          class="flex-1 bg-card rounded-lg border border-border overflow-hidden"
        >
          <div class="flex flex-col h-full">
            <!-- Editor Toolbar -->
            <div
              class="flex items-center justify-between p-4 border-b border-border bg-bg"
            >
              <div class="flex items-center space-x-2">
                <button
                  v-for="(action, index) in [
                    ...
                    {
                      icon: LinkIcon,
                      command: () => promptForLink(editor ?? null),
                      active: 'link',
                    },
                  ]"
                  :key="index"
            ...
            <div class="flex-1 overflow-y-auto p-6">
              ...
              <div class="space-y-4">
                ...
                <input
                  ...
                  @input="
                    (e) =>
                      (newNote.tags = (e.target as HTMLInputElement).value
                        .split(',')
                        .map((t) => t.trim()))
                  "
                />
                ...
                <input
                  ...
                  @input="
                    (e) =>
                      (newNote.keywords = (e.target as HTMLInputElement).value
                        .split(',')
                        .map((k) => k.trim()))
                  "
                />

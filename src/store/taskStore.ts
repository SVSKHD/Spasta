...
export const useTaskStore = defineStore('task', {
  ...
  getters: {
    ...
    totalHoursSpent: (state) => {
      return state.tasks.reduce((sum, task) => {
        ...
        return sum + timeEntries.reduce((total, entry) => total + entry.hours, 0);
      }, 0);
    }
  },
  ...
  actions: {
    ...
    async addTask(task: Omit<Task, 'id' | 'userId' | 'createdAt' | 'updatedAt'>) {
      const authStore = useAuthStore();
    ...
    async updateTask(taskId: string, updates: Partial<Omit<Task, 'id' | 'userId' | 'createdAt' | 'updatedAt'>>) {
      const authStore = useAuthStore();
      if (!authStore.user?.id) throw new Error('User not authenticated');
    ...
    async deleteTasksByCategory(categoryId: string) {
      ...
      try {
        const tasksToDelete = this.tasks.filter(t => t.categoryId === categoryId && t.userId === authStore.user?.id);
